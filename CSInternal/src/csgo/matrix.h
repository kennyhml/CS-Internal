...
struct Matrix3x4
{
	...
	constexpr Matrix3x4(const Vector3& x, const Vector3& y, const Vector3& z, const Vector3& origin)
	{
		Init(x, y, z, origin);
	}

	constexpr void Init(const Vector3& forward, const Vector3& left, const Vector3& up, const Vector3& origin)
	{
		SetForward(forward);
		SetLeft(left);
		SetUp(up);
	...
	constexpr void SetLeft(const Vector3& left)
	{
	...
	[[nodiscard]] constexpr Vector3 at(const int index) const
	{
...
struct ViewMatrix
{
	...
	constexpr ViewMatrix(const Matrix3x4& from, const Vector4& additionalColumn = { })
	{
	...
	[[nodiscard]] constexpr Vector4 at(const int index) const
	{
